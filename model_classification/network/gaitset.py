...
class SetNet(nn.Module):
    ...
    def _drop(self, x):
        if self.dropout is None:
            return x
        if x.dim() == 5:
            # (n, frames, c, h, w) → on fusionne batch et frames pour le dropout
            n, s, c, h, w = x.shape
            x = self.dropout(x.view(n * s, c, h, w))
            return x.view(n, s, c, h, w)
        else:
            # 4D (n, c, h, w) : cas des gl_layers
