...
class DataSet(tordata.Dataset):
    def __init__(self, seq_dir, label, seq_type, view, cache, resolution, patient_id=None):
        self.seq_dir = seq_dir
        self.view = view
        self.seq_type = seq_type
        self.label = label
        self.patient_id = patient_id if patient_id is not None else [None] * len(label)
        self.cache = cache
    ...
    def __getitem__(self, index):
        ...
        return data, frame_set, self.view[
            index], self.seq_type[index], self.label[index], self.patient_id[index]
